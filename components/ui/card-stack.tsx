...
export function CardStack<T extends CardStackItem>({
  ...
  // Clamp card dimensions to viewport on mobile to prevent overflow
  ...
  const cardWidth = Math.min(cardWidthProp, viewportWidth - 48);
...
function DefaultFanCard({ item }: { item: CardStackItem; active: boolean }) {
  const isHairToDreads = item.id === 1;
  ...
  return (
    <div className="relative h-full w-full overflow-hidden">
      ...
      <div className="absolute inset-0">
        {isHairToDreads ? (
          /* Hair to Dreads: Rasta gradient bg + centered portrait photo */
          <>
            {/* Rasta tricolor gradient background */}
            <div
              className="absolute inset-0"
              style={{
                background:
                  'linear-gradient(135deg, #C8102E 0%, #C8102E 25%, #009B3A 25%, #009B3A 55%, #FCD116 55%, #FCD116 100%)',
              }}
            />
      ...
      {/* Hair to Dreads logo overlay (Card 1 only) */}
      {isHairToDreads && (
      ...
      {item.tag && (
        <div className="absolute top-4 right-4 z-20">
          <span className="inline-flex items-center rounded-full bg-white/10 backdrop-blur-md px-3 py-1 text-xs font-semibold text-white border border-white/20 shadow-lg">
            {item.tag}
          </span>
        </div>
      )}
