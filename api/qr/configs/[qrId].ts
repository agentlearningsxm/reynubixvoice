...
import { getQrRoute, sanitizeQrId, upsertQrRoute } from '../../../lib/qrRouteStore';
...
function getQrId(req: VercelRequest): string | null {
  const raw = Array.isArray(req.query.qrId) ? req.query.qrId[0] : req.query.qrId;
  return sanitizeQrId(raw);
}

export default async function handler(req: VercelRequest, res: VercelResponse) {
  const qrId = getQrId(req);
  if (!qrId) return res.status(400).json({ error: 'Invalid qr id.' });
  const ip = getClientIp(req);

  if (req.method === 'GET') {
    const readRateLimit = enforceRateLimit({
      key: `qr-config-read:${ip}:${qrId}`,
      limit: 240,
      windowMs: 60 * 1000
    });
    if (!readRateLimit.ok) {
    ...
    const result = await getQrRoute(qrId);
    if (!result.data) {
      return res.status(404).json({ error: 'QR config not found.', source: result.source });
    }
  ...
  if (req.method === 'PUT' || req.method === 'PATCH') {
    ...
    try {
      ...
      const result = await upsertQrRoute(qrId, {
