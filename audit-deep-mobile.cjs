/**
 * Deep Mobile UI/UX Audit — Baseline Measurement
 * Target: http://192.168.178.179:3000 (Docker production build)
 * Viewports: 320, 375, 390, 412 × 812
 *
 * Run: node audit-deep-mobile.cjs
 */
...
// The audit function — passed as a real function to page.evaluate with args
function runAudit(cfg) {
  ...
  // 6. Carousel cards — find flex-[0_0_X%] elements
  var carouselCards = [];
...
(async () => {
  ...
  console.log('============================================================');
  console.log('     DEEP MOBILE UI/UX AUDIT - Baseline Measurement');
  console.log('============================================================');
  ...
  for (var vi = 0; vi < VIEWPORTS.length; vi++) {
    ...
    try {
      ...
      var fullPath = path.join(OUTPUT_DIR, `baseline-${label}-full.png`);
      ...
      for (var si = 0; si < sectionHandles.length; si++) {
        ...
        var secPath = path.join(
          OUTPUT_DIR,
          `baseline-${label}-section-${si}-${cleanId}.png`,
        );
      ...
      // Run all checks via evaluate with args (no string escaping issues)
  ...
  var reportLines = [];
  reportLines.push('# Mobile Deep Audit - Baseline Report');
  reportLines.push('');
  ...
  for (var reci = 0; reci < allFails.length; reci++) {
    ...
    if (rf.cat === 'touch-targets')
      recSet.touch =
        '- **Touch targets:** Increase all clickable elements to >= 44x44px. Add padding or min-width/min-height as needed.';
    if (rf.cat === 'horizontal-scroll')
      recSet.hscroll =
        '- **Horizontal overflow:** Add overflow-x: hidden to offending sections or fix elements extending beyond viewport.';
    if (rf.cat === 'h2-overflow')
      recSet.h2 =
        '- **H2 overflow:** Use text-wrap: balance, reduce font-size on mobile, or add horizontal padding.';
    if (rf.cat === 'carousel-cards')
      recSet.cards =
        '- **Carousel cards:** Adjust flex basis to be between 70-90% of viewport for optimal mobile display.';
    if (rf.cat === 'carousel-arrows')
      recSet.arrows =
        '- **Carousel arrows:** Add margin to carousel container to prevent arrow overlap with cards.';
    if (rf.cat === 'calculator-number')
      recSet.calc =
        '- **Calculator number:** Use responsive font sizing (e.g. clamp()) or overflow-x: auto on the number container.';
    if (rf.cat === 'hero-phone')
      recSet.phone =
        '- **Hero phone mockup:** Ensure phone element scales down enough to not exceed viewport width on small screens.';
    if (rf.cat === 'hero-cta')
      recSet.cta =
        '- **Hero CTA:** Ensure w-full class is applied on mobile for full-width CTA button.';
    if (rf.cat === 'footer-bottom')
      recSet.footer =
        '- **Footer bottom bar:** Stack copyright and back-to-top vertically or use justify-between with proper spacing.';
    if (rf.cat === 'mask-depth')
      recSet.mask =
        '- **Mask depth:** Reduce mask gradient extent to lose <= 10% of section height on mobile.';
    if (rf.cat === 'section-spacing')
      recSet.spacing =
        '- **Section spacing:** Fix overlapping sections or reduce excessive gaps (> 120px) for mobile flow.';
  }
  var recKeys = Object.keys(recSet);
  if (recKeys.length === 0) {
    reportLines.push(
      'No critical issues found. Site passes all baseline checks across tested viewports.',
    );
  } else {
  ...
  // Write report
  var reportPath = path.join(OUTPUT_DIR, 'audit-report.md');
  fs.writeFileSync(reportPath, reportLines.join('\n'));
  console.log(`Report -> ${reportPath}`);

  // Write raw JSON data
  var jsonPath = path.join(OUTPUT_DIR, 'audit-raw-data.json');
